...
- **Deploy - preparar bcb.yanlucas.com com frontend em container**:
  ...
  - Hotfix CI após falha no PR: em runner limpo, `pnpm test` falhava antes de `pnpm build` porque `@bcb/shared` exporta `dist/*` e o Jest da API não encontrava `@bcb/shared`; `apps/api/package.json` agora tem `pretest` e `pretest:db` rodando `pnpm --filter @bcb/shared build` antes do Jest.
  - Hotfix deploy após falha no runner: containers subiam e API respondia `{"service":"@bcb/api","status":"ok"}`, mas o smoke test batia no web em `4220` imediatamente após `bcb-web-1 Started` e podia receber `curl: (56) Recv failure: Connection reset by peer`; workflow agora espera o web com retry antes do `curl` final.
  ...
  - RED/GREEN específico do CI: com `packages/shared/dist` movido para `/tmp`, `pnpm test` reproduziu `Cannot find module '@bcb/shared'`; após o `pretest`, `pnpm test` passou e `pnpm --filter @bcb/api test -- queue.spec.ts` também executou o build prévio do shared sem erro de resolução.
...
- `@bcb/shared` publica `exports` apontando para `dist`; em checkout limpo, testes da API precisam compilar o shared antes do Jest. Manter `pretest`/`pretest:db` em `apps/api/package.json`.
